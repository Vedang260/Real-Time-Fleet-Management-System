...
// Vehicle ID (assuming passed via route or prop)
const vehicleId = computed(() => route.params.vehicleId as string) // Replace with actual vehicleId from route or props
...
const saveLocations = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      `http://localhost:8000/routes/${vehicleId.value}`,
      routeData,
      {
        headers: {
